...
def build_crosscheck_key(client_id: pd.Series, service_date: pd.Series, procedure: pd.Series) -> pd.Series:
    """Match SDR to county using Client ID + Date + normalized Procedure.

    Minutes and units are intentionally NOT included because those are what we compare.
    """
    ...
    clean_proc = procedure.apply(normalize_proc_for_crosscheck)
    return clean_ids + "|" + clean_dates + "|" + clean_proc
...
def load_sdr_for_crosscheck(file_bytes: bytes) -> pd.DataFrame:
    ...
    if "Procedure Code Name" not in df.columns:
        raise ValueError("SDR cross-check needs Procedure Code Name.")

    # Support both Face-to-Face spellings.
    ...
    if missing:
        raise ValueError("SDR cross-check is missing required column(s): " + ", ".join(missing))
    ...
    df["Match Key"] = build_crosscheck_key(df[client_id_col], df[date_col], df["Procedure Code Name"])
...
def load_county_for_crosscheck(file_bytes: bytes) -> pd.DataFrame:
    # First try normal header reading. Some county exports have headers.
    ...
    county["Match Key"] = build_crosscheck_key(county[client_id_col], county[date_col], county[proc_col])
...
def run_county_crosscheck(sdr_file_bytes: bytes, county_file_bytes: bytes) -> Dict[str, Any]:
    ...
    # Big-picture totals by procedure code.
    app_summary = (
    ...
    # Correct reconciliation: aggregate by Client ID + Date + Procedure.
    # This avoids false missing rows when multiple services exist for the same client/date.
    sdr_keyed = (
        sdr.groupby(["Match Key", "Crosscheck ClientId", "Crosscheck Date", "Crosscheck Procedure"], dropna=False)
        .agg(SDR_Rows=("Match Key", "size"), App_Minutes=("Face-to-Face Time", "sum"), App_Units=("App Units", "sum"))
        .reset_index()
    )
    county_keyed = (
        county.groupby(["Match Key", "Crosscheck ClientId", "Crosscheck Date", "Crosscheck Procedure"], dropna=False)
        .agg(County_Rows=("Match Key", "size"), County_Minutes=("County Minutes", "sum"), County_Charge_Units=("County Charge Units", "sum"))
        .reset_index()
    )

    matched = sdr_keyed.merge(
        county_keyed[["Match Key", "County_Rows", "County_Minutes", "County_Charge_Units"]],
        on="Match Key",
        how="left",
        indicator=True,
    )
    matched["County_Rows"] = pd.to_numeric(matched["County_Rows"], errors="coerce").fillna(0).astype(int)
    matched["County_Minutes"] = pd.to_numeric(matched["County_Minutes"], errors="coerce").fillna(0)
    matched["County_Charge_Units"] = pd.to_numeric(matched["County_Charge_Units"], errors="coerce").fillna(0)
    matched["Unit Difference"] = matched["App_Units"] - matched["County_Charge_Units"]
    ...
    show_cols = [
        "Crosscheck Date", "Crosscheck ClientId", "Crosscheck Procedure",
        "SDR_Rows", "App_Minutes", "App_Units",
        "County_Rows", "County_Minutes", "County_Charge_Units", "Unit Difference"
    ]
...
def render_county_crosscheck_report(report: Dict[str, Any]) -> None:
    ...
    st.info("This compares SDR submitted services against the county billed/accepted file. Row matching uses Client ID + Date + Procedure: County ClientId ↔ SDR Client or Group Id, and County DateOfService ↔ SDR Service Date. Minutes and units are compared after matching.")
    ...
    c4.metric("Exact Matches", report.get("exact_row_count", 0))
    c5.metric("Adjusted Matches", report.get("adjusted_row_count", 0))
    c6.metric("Truly Missing", report.get("missing_row_count", 0))
    ...
    st.markdown("#### Adjusted Matches — Found in County, But Units Differ")
    if adjusted is None or adjusted.empty:
        st.success("No adjusted unit matches found.")
    else:
        st.dataframe(adjusted, use_container_width=True, hide_index=True)
        st.download_button(
            "Download Adjusted Matches CSV",
            data=adjusted.to_csv(index=False).encode("utf-8"),
            file_name="sdr_adjusted_matches_county.csv",
            mime="text/csv",
        )

    missing = report.get("missing")
    st.markdown("#### Truly Missing — Submitted SDR Rows Not Found in County")
    if missing is None or missing.empty:
        st.success("No truly missing SDR rows found with Client ID + Date matching.")
    else:
        st.dataframe(missing, use_container_width=True, hide_index=True)
        st.download_button(
            "Download Truly Missing Rows CSV",
            data=missing.to_csv(index=False).encode("utf-8"),
            file_name="sdr_truly_missing_from_county.csv",
            mime="text/csv",
        )
