...
def compute_pass(hours_worked: float, file_bytes: bytes) -> Results:
    minutes_worked_raw = hours_worked * 60.0

    df, _ = load_excel_auto_header(file_bytes, dtype=object)

    mapping = canonicalize_headers(list(df.columns))
...
with col_reset:
    st.button("Run Another Staff Member", on_click=do_reset)
...
if run:
    ...
    try:
        pass1 = compute_pass(hours_worked, file_bytes)
    ...
    try:
        pass2 = compute_pass(hours_worked, file_bytes)
    except Exception as e:
